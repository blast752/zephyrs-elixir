...
internal static class LicenseChecksum
{
    internal static string Sha256Base64(string data) =>
...
public static class LicenseConfig
{
    ...
    /// <summary>How far the server's signing time may sit from ours, in either direction. Generous
    /// on purpose: a desktop clock drifts, and a few minutes of skew must never cost a paying user
    /// their licence.</summary>
    public const int TimestampToleranceMinutes = 30;

    /// <summary>Grace applied to <see cref="LicenseState.ExpiresAt"/>, which carries the subscription's
    /// next renewal date rather than a true end date. Without it every renewal would revoke Pro until
    /// the next validation caught up; it comfortably outlasts the online validation interval.</summary>
    public const int RenewalGraceHours = 26;
}
...
public sealed record LicenseState
{
    ...
    public bool IsExpired => ExpiresAt.HasValue &&
        ExpiresAt.Value.AddHours(LicenseConfig.RenewalGraceHours) < DateTime.UtcNow;
...
internal sealed record CachedLicense
{
    ...
    public static string ComputeChecksum(string? normalizedKey, LicenseTier tier, LicenseStatus status, DateTime? expiresAt, string? signature, string? deviceId, string? proDllVersion = null)
    {
        ...
        return LicenseChecksum.Sha256Base64(data);
    }
...
public static class Features
{
    ...
    // Gating and execution must name the same thing. These features are entry points into the Pro
    // module, so their ids are its command ids — restated as literals they could drift apart, and
    // the licence check would then guard an id the module never receives.
    public const string ScreenMirror = ProCommandIds.ScreenMirrorPage;
    public const string PerformanceMonitor = ProCommandIds.PerformanceMonitorPage;
    public const string ApkBackup = ProCommandIds.ApkBackup;
    public const string PrivacySafetyCore = ProCommandIds.SafetyCore;
    public const string PrivacyAdId = ProCommandIds.ResetAdId;
    public const string PrivacyCaptivePortal = ProCommandIds.CaptivePortal;
    public const string PrivacyGoogleCore = ProCommandIds.GoogleCoreControl;
    public const string PrivacyRamExpansion = ProCommandIds.RamExpansion;
