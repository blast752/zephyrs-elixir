...
public sealed class DeviceManager
{
    ...
    private async Task UpdateAsync()
    {
        try
        {
            ...
            if (IsConnected)
            {
                var (battery, name, serial) = await FetchDeviceInfoAsync();
                if (battery != BatteryLevel || name != DeviceName || serial != DeviceSerial)
                {
    ...
    /// <summary>
    /// Single batched shell round-trip for the 2s polling tick: battery, brand, model and
    /// serial in one adb.exe spawn instead of four. Parsing semantics are unchanged.
    /// </summary>
    private static async Task<(int Battery, string Name, string Serial)> FetchDeviceInfoAsync()
    {
        const string Sep = "ZE-SEP";
        var output = await AdbExecutor.ExecuteCommandAsync(
            $"shell \"dumpsys battery; echo {Sep}; getprop ro.product.brand; echo {Sep}; getprop ro.product.model; echo {Sep}; getprop ro.serialno\"",
            log: false);
        ...
        if (string.IsNullOrWhiteSpace(name)) name = Strings.DeviceStatus_NoDevice;

        var serial = sections.Length > 3 ? Clean(sections[3]) : string.Empty;
        if (string.IsNullOrEmpty(serial))
            serial = Clean(await AdbExecutor.ExecuteCommandAsync("get-serialno", log: false));

        return (battery, name, serial);
    }
...
public static class ZephyrsAgent
{
    ...
    public static async Task<bool> EnsureAgentIsRunningAsync(IProgress<string>? progress = null, CancellationToken ct = default)
    {
        ...
        try
        {
            if (_running)
            {
                try { await HttpClient.GetAsync($"{BaseUri}/apps", ct); return true; }
            ...
            try
            {
                await HttpClient.GetAsync($"{BaseUri}/apps", ct);
                _running = true;
                progress?.Report("Agent ready.");
                return true;
            }
